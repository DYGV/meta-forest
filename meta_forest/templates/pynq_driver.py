...
class FpgaDriver:
    def __init__(
        ...
        self.overlay.is_loaded = self._is_loaded
    ...
    def do_calc(self):
        self._cmd_stream_transfer()
        self._cmd_burst_transfer()
    ...
    def process_input(self, signal_name, input_data):
        ...
        if protocol == "stream":
            ...
            if is_array:
                if signal_type == "int":
                    if data_width == 8:
                        self._write_uint8_array_lite(signal, input_data)
                    elif data_width == 32:
                        self._write_uint32_array_lite(signal, input_data)
                elif signal_type == "float":
                    if data_width == 32:
                        self._write_float32_array_lite(signal, input_data)
                    elif data_width == 64:
                        self._write_float64_array_lite(signal, input_data)
            else:
                if signal_type == "int":
                    ...
                    if data_width == 32:
                        ieee_rep = self._dec_to_ieee(input_data, "float")
                        self.user_ip.write(address_offset, ieee_rep)
                    elif data_width == 64:
                        ieee_rep_hi, ieee_rep_lo = self._dec_to_ieee(
                            input_data, "double"
                        )
    ...
    def process_output(self, signal_name):
        ...
        if protocol == "stream":
            ...
            if is_array:
                if signal_type == "int":
                    if data_width == 8:
                        return self._read_uint8_array_lite(signal)
                    elif data_width == 32:
                        val = self._read_int32_array_lite(signal)
                        return val
                elif signal_type == "float":
                    if data_width == 32:
                        return self._read_float32_array_lite(signal)
                    elif data_width == 64:
                        return self._read_float64_array_lite(signal)

    def _read_uint8_array_lite(self, signal):
    ...
    def _read_int32_array_lite(self, signal):
        array_size = signal.get("array_size")
        val = [0] * array_size
    ...
    def _read_float32_array_lite(self, signal):
        array_size = signal.get("array_size")
        val = [0] * array_size
        for i in range(array_size):
    ...
    def _read_float64_array_lite(self, signal):
        array_size = signal.get("array_size")
        val = [0] * array_size
    ...
    def _write_uint8_array_lite(self, signal, msg):
        array_size = signal.get("array_size")
        array_head = signal.get("address_offset")
    ...
    def _write_uint32_array_lite(self, signal, msg):
        array_size = signal.get("array_size")
        array_head = signal.get("address_offset")
        for i in range(array_size):
            self.user_ip.write(array_head + (4 * i), int(msg[i]))

    def _write_float32_array_lite(self, signal, msg):
        array_size = signal.get("array_size")
        array_head = signal.get("address_offset")
    ...
    def _write_float64_array_lite(self, signal, msg):
        array_size = signal.get("array_size")
        array_head = signal.get("address_offset")
    ...
    def _dec_to_ieee(self, num, data_type):
    ...
    def _ieee_to_dec(self, num_ieee, data_type):
